...
then
	...
	#GIT_PULL_SYMBOL="↓"
	GIT_PULL_SYMBOL="v"
	if [[ ${PROMPT_GIT:-0} -eq 1 && ${PROMPT_GIT_REMOTE:-0} -eq 1 ]] && git rev-parse --git-dir &>/dev/null && [ $(git pull --dry-run 2>&1 | wc -l) -gt 1 ]
	then
...
#GIT_PUSH_SYMBOL="↑"
GIT_PUSH_SYMBOL="^"
if [[ ${PROMPT_GIT:-0} -eq 1 && ${PROMPT_GIT_REMOTE:-0} -eq 1 ]] && git rev-parse --git-dir &>/dev/null
then
...
if [[ ${PROMPT_GIT:-0} -eq 1 && ${PROMPT_GIT_REPO:-0} -eq 1 ]] && git rev-parse --git-dir &>/dev/null
then
...
if [[ ${PROMPT_GIT:-0} -eq 1 && ${PROMPT_GIT_BRANCH:-0} -eq 1 ]] && git rev-parse --git-dir &>/dev/null
then
	if [ ${PROMPT_GIT_SYMBOLS:-0} -eq 1 ]
	then
		GIT_NEW_FILE_SYMBOL="+"
		GIT_REM_FILE_SYMBOL="-"
	...
	then
		...
		git status -s | cut -c1-2 | grep -q -m 1 " M" && printf "\[\033[1;38;5;1m\]${GIT_EDIT_FILE_SYMBOL}"
		git status -s | cut -c1-2 | grep -q -m 1 "??" && printf "\[\033[1;38;5;1m\]${GIT_NEW_FILE_SYMBOL}"
		git status -s | cut -c1-2 | grep -q -m 1 " D" && printf "\[\033[1;38;5;1m\]${GIT_REM_FILE_SYMBOL}"
	fi
fi
`'
